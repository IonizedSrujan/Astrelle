...
def run_simulation(tel_key, sen_key, dt_utc, exposure_s,
    ...
    # Helper function for logging messages to the GUI.
    def log(msg):
        if log_callback:
            log_callback(f"[{datetime.now():%H:%M:%S}] {msg}")

    # Helper function for updating the progress bar in the GUI.
    def update_progress(val):
        if progress_callback:
            progress_callback(val)

    # Load telescope and sensor presets.
    presets = background.load_presets()
    tel = presets["telescopes"][tel_key]
    sen = presets["sensors"][sen_key]
    nx, ny = sen["resolution"]
    
    # Convert pointing from Alt/Az to RA/Dec if needed.
    ...
    # Calculate image properties like pixel scale and field of view.
    ...
    if not galaxies_df.empty:
        ...
        galaxies_df = pd.DataFrame() # Ensure dataframe exists even if empty.

    sats_out_df = pd.DataFrame()

    # Handle satellite trail generation based on selected mode.
    if sat_mode == "Catalog":
