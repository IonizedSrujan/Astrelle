...
def get_data_path(filename):
    """Gets the full path to a data file within the package."""
    return pkg_resources.resource_filename('astrelle', f'data/{filename}')

# Use the helper to find the absolute path to data files
...
def save_presets(presets: dict):
    # NOTE: In a real app, user presets would save to a user directory like ~/.config/astrelle
    # For now, this will attempt to write back to the package data, which may fail due to permissions.
    # The primary goal of this fix is to resolve the loading error.
...
def load_passband_data():
    ...
    if _passband_cache is None:
        ...
        _passband_cache = df[df['GPb'] < 99.0].copy()
