...
# Worker thread for running the simulation without freezing the GUI.
...
class AddPresetDialog(QDialog):
    def __init__(self, mode="telescope", parent=None):
        ...
        # Add fields specific to telescope presets.
        if mode == "telescope":
        ...
        # Add fields specific to sensor presets.
        else:
            self.fields.update({k: QLineEdit(v) for k, v in {
...
class SyntheticTrailEditor(QWidget):
    ...
    # Show/hide fields based on manual vs. random generation mode.
...
class SkySimGUI(QMainWindow):
    ...
    def _build_ui(self):
        ...
        # Setup main layout with a splitter.
        ...
        # Observation parameters groupbox.
        ...
        # Satellite trail simulation groupbox.
        ...
        # Right panel for image display and log.
        ...
        self.toolbar.setStyleSheet("background-color: #383838;") # Match theme
    ...
    # Initialize the plot area before a simulation is run.
    def _init_plot(self):
        self.fig.clear()
    ...
    # Show/hide random generation fields for synthetic trails.
    def toggle_synth_gen_mode(self, mode):
        is_manual = mode == "Manual"
        for editor in self.trail_editors:
            editor.set_manual_mode(is_manual)

    # Add or remove synthetic trail editor widgets based on user input.
    def update_trail_editors(self, count):
        ...
        # Show a dropdown to select between editors if there are multiple.
    ...
    # Switch the visible trail editor in the stacked widget.
    def switch_trail_editor(self, index):
        if index >= 0:
            self.trail_editors_stack.setCurrentIndex(index)

    # Show/hide UI elements based on pointing mode (Alt/Az vs RA/Dec).
    ...
    # Update the text boxes with info about the selected telescope/sensor.
    ...
    # Collect all simulation parameters from the GUI.
    def _get_params(self):
        ...
        if params["pointing_mode"] == "Local (Alt/Az)":
            ...
            params.update({
                ...
                "ra_deg": float(self.edits['ra'].text()),
                "dec_deg": float(self.edits['dec'].text())
            })
        return params
    ...
    # Redraw the plot with the new simulation data.
    def redraw_plot(self):
        ...
        # Create axes with WCS projection if available.
        if wcs:
        ...
        # Add annotations using scatter plots for better scaling on zoom.
    ...
    # Export the current image as PNG or FITS.
    ...
    # Export object catalogs as CSV files.
