...
MAX_WORKERS = 8 # Max threads for satellite processing.
REF_ALT_KM = 400.0 # Reference altitude for magnitude calculation.
REF_MAG = 1.0 # Reference magnitude at reference altitude.
...
# Downloads the latest active satellite TLEs from CelesTrak.
def download_tles(force=False, timeout=30, cache_dir=Path(".")):
    tle_cache_path = cache_dir / "celestrak_tles.txt"
    # Use cached file if it exists and is not forced to re-download.
...
def parse_tles(txt):
    ...
    while i + 2 < len(lines):
        name = lines[i].strip()
        # Check for the valid 3-line TLE format.
...
# Calculates a satellite's RA/Dec coordinates at a specific time.
...
def _cohen_sutherland_clip(x0, y0, x1, y1, x_min, y_min, x_max, y_max):
    ...
    while True:
        if not (code0 | code1): return True # Both points inside.
        if code0 & code1: return False # Both points outside on the same side.
        dy, dx = y1 - y0, x1 - x0
...
# Finds all satellites from the TLE catalog that pass through the FOV during the exposure.
def get_sats_in_fov(presets, tel_key, dt_utc, exposure_s, fov_arcmin, center_ra_dec, progress_callback=None, log_callback=None, cache_dir=Path(".")):
    ...
    # Define a larger search radius to pre-filter satellites.
    ...
    # Function to check a single TLE, designed to be run in a separate thread.
    def check_tle(tle):
        try:
            ...
            # Pre-filter using a simple spherical separation check.
            ...
            # Calculate start and end points of the trail.
            ...
            # Correct for tracking drift.
            ra1_corrected = ra1 - frame_drift_deg
            
            # Use Cohen-Sutherland clipping for precise FOV check.
...
def render_satellite_trails(image, sats_df, wcs, zero_point, exposure_s, tel_key, sen_key, presets, dt_utc, seeing_fwhm_arcsec, pixel_scale):
    ...
    num_frames = int(exposure_s * fps) if exposure_s * fps > 1 else 1
    time_per_frame = exposure_s / num_frames
    ...
    for _, sat_info in sats_df.iterrows():
        ...
        # Calculate apparent magnitude based on altitude.
        mid_time = dt_utc + timedelta(seconds=exposure_s / 2.0)
        geocentric = sat.at(ts.from_datetime(mid_time))
        alt_km = wgs84.height_of(geocentric).km
        apparent_mag = mag_from_altitude_km(alt_km)
        if apparent_mag > MAG_LIMIT: continue
            
        # Calculate flux and get PSF stamp.
        ...
        # Get start and end coordinates of the trail in RA/Dec.
        ra0, dec0, _ = _get_sat_coords_at_time(sat, topos, dt_utc)
        ra1, dec1, _ = _get_sat_coords_at_time(sat, topos, dt_utc + timedelta(seconds=exposure_s))
        ...
        # Add a PSF stamp for each simulated sub-frame of the exposure.
...
def generate_synthetic_trails(image, wcs, trail_params_list, pixel_scale, zero_point, exposure_s, fps):
    ...
    for i, trail_params in enumerate(trail_params_list):
        ...
        # Calculate flux and get PSF.
        flux_e_per_sec = zero_point * 10**(-0.4 * apparent_mag)
        flux_per_frame = flux_e_per_sec * time_per_frame
        psf_stamp = gaussian_psf_kernel(fwhm_pix=trail_fwhm_arcsec / pixel_scale)

        # Convert start coordinates to pixels and calculate velocity vector.
        start_coords_pix = wcs.all_world2pix([[start_ra_deg, start_dec_deg]], 0)[0]
        x_start, y_start = start_coords_pix[0], start_coords_pix[1]
        angle_rad = np.deg2rad(90.0 - angle_deg) # Convert from E of N to standard angle.
        ...
        # Add a PSF stamp for each sub-frame.
...
# Singleton pattern for the expensive Skyfield timescale object.
_timescale = None
def get_timescale():
    global _timescale
